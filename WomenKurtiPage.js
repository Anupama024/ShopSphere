...
function WomenKurtiPage() {
  ...
  // Get women kurti products and kids t-shirts from global productDetails
  const womenKurtiProducts = productDetails.filter(p => (p.id >= 801 && p.id <= 830) || (p.id >= 1101 && p.id <= 1145));
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', bgcolor: '#fafafa' }}>
      <GlobalAppBarDrawer
        ...
        // Navigation for Women > Kurtis in drawer
      />
