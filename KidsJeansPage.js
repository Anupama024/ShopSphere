...
function KidsJeansPage() {
  const [searchTerm, setSearchTerm] = React.useState('');
  const [filterAnchorEl, setFilterAnchorEl] = React.useState(null);
  const [selectedFilters, setSelectedFilters] = React.useState({ Boy: false, Girl: false, Formal: false, Casual: false });
  const [pendingFilters, setPendingFilters] = React.useState({ Boy: false, Girl: false, Formal: false, Casual: false });
  ...
  // Get kids jeans products from global productDetails (category: 'KidsJeans')
  const kidsJeansProducts = productDetails.filter(p => p.category === 'KidsJeans');
  ...
  const handleClearFilters = () => {
    setPendingFilters({ Boy: false, Girl: false, Formal: false, Casual: false });
    setPendingPrice([100, 2000]);
    setPendingAge([0, 15]);
    setSelectedFilters({ Boy: false, Girl: false, Formal: false, Casual: false });
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', bgcolor: '#fafafa' }}>
      <GlobalAppBarDrawer
        ...
        // Navigation for Kids > Jeans in drawer
      />
        ...
        <Grid container spacing={3} sx={{ m: 0, width: '100%', maxWidth: 1400, mx: 'auto', pr: { xs: 1, sm: 3, md: 6 } }}>
          {kidsJeansProducts
            .filter(product => {
              ...
              // Gender filter
              const genderActive = Object.keys(selectedFilters).filter(k => selectedFilters[k] && (k === 'Boy' || k === 'Girl'));
              const inGender = genderActive.length === 0 || genderActive.includes(product.category) || genderActive.includes(product.gender);
              // Type filter (Formal/Casual)
              const typeActive = Object.keys(selectedFilters).filter(k => selectedFilters[k] && (k === 'Formal' || k === 'Casual'));
              const inType = typeActive.length === 0 || typeActive.includes(product.type);
              ...
              return matchesSearch && inGender && inType && inPrice && inAge;
            })
            .map(product => {
