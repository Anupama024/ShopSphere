...
function MenPantsPage() {
  ...
  // Get pants products from global productDetails
  const pantsProducts = productDetails.filter(p => p.id >= 401 && p.id <= 413);
          ...
          {pantsProducts
            ...
            .map(product => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={product.id}>
                <GlobalProductCard product={product} onClick={() => handleProductClick(product)} />
              </Grid>
            ))}
        </Grid>
        <GlobalProductDialog
